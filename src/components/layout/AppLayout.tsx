...
const AppLayout = () => {
        ...
        {navItems.map(item => {
          ...
          return (
            <NavLink key={item.path} to={item.path}
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-150 relative group ${active
  ...
  return (
    <div className="flex flex-col h-screen bg-background">
      {/* Header */}
      <header className="h-16 flex items-center justify-between px-5 border-b border-border bg-background shrink-0 z-30">
        ...
        <div className="flex items-center gap-5">
          <span className="font-ui text-[13px] text-rx-text-secondary hidden sm:inline">
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Desktop Sidebar */}
        <aside className={`hidden lg:flex flex-col border-r border-border bg-background shrink-0 transition-all duration-200 relative ${sidebarOpen ? 'w-56' : 'w-[60px]'
          }`}>
