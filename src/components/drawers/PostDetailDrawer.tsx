    ...
    <div className="fixed right-0 top-0 bottom-0 w-full max-w-[480px] bg-rx-surface border-l border-border z-50 flex flex-col animate-slide-in-right">
      ...
      <div className="flex-1 overflow-auto p-5 space-y-4">
        {post.content_type !== 'text' && (
          <div className="w-full aspect-video rounded-xl bg-rx-elevated flex items-center justify-center text-rx-text-muted font-ui text-[13px]">
            {post.content_type === 'photo' ? '📷 Image' : '🎬 Vidéo'}
          </div>
        )}

        <div className="card-elevated p-4">
          <p className="font-ui text-[13px] text-foreground leading-relaxed">{post.text}</p>
        </div>
      ...
      <div className="p-4 border-t border-border flex gap-3">
        <button className="flex-1 h-10 bg-[hsl(0_47%_11%)] text-rx-danger font-ui font-medium text-[13px] rounded-lg hover:bg-[hsl(0_47%_15%)] transition-colors">
